...
combine_and_sort <- function(day_list, hour_list, info_list) {
  ...
  #combined_day  <- unlist(day_list,  use.names = FALSE)  # instead of do.call(c, ...)
  #combined_hour <- unlist(hour_list, use.names = FALSE)  # instead of do.call(c, ...)
...
generate_descriptive_stats_table <-
  function(datafiles, columnName, renameName, monitorInfo, title = "Sample Title - Please fill") {
    ...
    tb <- flextable(result) %>%
      ...
      bg(j = "Median", bg = scales::col_numeric(light_blue_palette, domain = median_domain)(result$Median)) %>%
      bg(j = "Minimum", bg = scales::col_numeric(light_yellow_palette, domain = minimum_domain)(result$Minimum)) %>%
      bg(j = "Maximum", bg = scales::col_numeric(light_red_palette, domain = maximum_domain)(result$Maximum))
