...
# 2. Get and analyze past month HOURLY data from Clarity sensors ####
# make request - hourly
clarity_last_month <-
  purrr::map(
    .x = uniqueOrgID,
...
clarity_last_month_data_clean <-
  split_clarity_data_by_datasource(clarity_last_month, deviceId) %>%
  purrr::list_flatten() %>%
  setNames(deviceId)
...
graphTest <- purrr::reduce2(
  ...
  .f = function(acc, x, y) {
    ...
    # Compute results
    results <- list(
      "PM2.5 Graph" = test_sensor_malfunction_result(x, "startOfPeriod", "pm2_5ConcMass1HourMean.value", start_time_ISO, end_time_ISO),
      "NO2 Graph" = test_sensor_malfunction_result(x, "startOfPeriod", "no2Conc1HourMean.value", start_time_ISO, end_time_ISO),
      "Temperature Graph" = test_sensor_malfunction_result(x, "startOfPeriod", "temperatureInternal1HourMean.raw", start_time_ISO, end_time_ISO),
      "Humidity Graph" = test_sensor_malfunction_result(x, "startOfPeriod", "relHumidInternal1HourMean.raw", start_time_ISO, end_time_ISO)
    )
...
processed_report <- list(
  ...
  StatusCheck = clarity_status_data_clean %>% 
    full_join(monitor_info_Clarity, by = dplyr::join_by(DeviceID)),
  GraphCheck = graphTest %>%
    full_join(monitor_info_Clarity, by = dplyr::join_by(sensor_index == DeviceID)),
