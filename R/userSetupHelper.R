...
setup_helper <- function(testing = FALSE) {
  ...
  if (testing) {
    cat("⚠️  WARNING: TESTING MODE ENABLED ⚠️\n")
  ...
  cat("⚠️  WARNING: The following environment variables will dictate where:\n")
  ...
  if (testing) {
    env_file <- ".RTestEnviron"  # Current directory for testing
  } else {
  ...
  cat("⚠️  IMPORTANT: Please restart R to load the environment variables properly.\n")
