...
CreateMonthlyDisplayTable <- function(QuestionMainCode, SensorMainCode, isNoteTable = F) {
  ...
  if (isNoteTable) {
    pInfo <- pInfo %>%
      dplyr::filter(!dplyr::if_any(dplyr::contains("Notes"), ~ (is.na(.) | . == ""))) 
  }
  
  tb <- flextable::flextable(pInfo) %>%
  ...
  for (col in pColumnName) {
    ...
    tb <- tb %>% flextable::bg(i = which(highlight_rows), j = col, bg = "#FBEC5D")
  }
  
  # Based on Dr. Stuart modifications
  if ("Tracking Summary" %in% names(pInfo)) {
    tb <- tb %>% flextable::set_header_labels(`Tracking Summary` = "Tracking Follow-up Needed")
  }
  
  tb <- FitFlextableToPage(tb)

  # Change font sizes & widths
  tb <- tb %>% flextable::width(j = 1, width = 1)
  if (isNoteTable) {
    tb <- tb %>% flextable::width(j = 2, width = 1)
    tb <- tb %>% flextable::width(j = 3, width = 4)
  }
...
tb <- flextable::flextable(paMerger) %>%
  #flextable::set_table_properties(layout = "autofit") %>%
  #flextable::border_inner_h(part = "all", border = small_border) %>%
  FitFlextableToPage() %>%
  flextable::border_inner_v(part = "all", border = small_border) %>%
...
for (col in colnames(paMerger)) {
  ...
  tb <- tb %>% flextable::bg(i = which(highlight_rows), j = col, bg = "#fb6768")
}
tb
```
...
tb <- flextable::flextable(clMerger) %>%
  #flextable::set_table_properties(layout = "autofit") %>%
  #flextable::border_inner_h(part = "all", border = small_border) %>%
  FitFlextableToPage() %>%
  flextable::border_inner_v(part = "all", border = small_border) %>%
...
pInfo <- params$myData$UnresolvedNotes %>%
  dplyr::mutate(OriginDate = paste(lubridate::month(OriginDate), lubridate::year(OriginDate), sep = "-")) %>% 
...
tb <- flextable::flextable(pInfo) %>%
  #flextable::set_table_properties(layout = "autofit") %>%
  flextable::bg(i = ~ Resolved == "Yes", j = "Resolved", bg = "lightgreen") %>%
  flextable::bg(i = ~ Resolved == "No", j = "Resolved", bg = "#fb6768") %>%
  #flextable::border_inner_h(part = "all", border = small_border) %>%
  flextable::border_inner_v(part = "all", border = small_border) 
...
tb <- tb %>%
  ...
  flextable::width(j = 3, width = 0.75) %>%
  flextable::width(j = 4, width = 1.3) %>%
  flextable::width(j = 5, width = 0.65) %>%
  flextable::width(j = 6, width = 2.6)
