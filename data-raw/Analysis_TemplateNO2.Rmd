...
params:
  ...
  myData: "`r list()`"
  debugTurnOn: TRUE
  start_date: '`r as.Date("2026-04-01")`'
...
trends_graph <- purrr::map2(
  ...
  .f = function(x, y) {
    plot <- generate_sitebase_scatter_plot_data(
      ...
      dplyr::bind_rows(params$myData[["Info"]], params$myData$ReferenceInfo),
      x,
...
trends_graph <- purrr::map(
  ...
  .f = function(x) {
    sub <- params$myData[["CombinedInfoRefCoNoPA"]] %>% dplyr::filter(DeviceID == !!x) %>% dplyr::pull(Subtype)
    ...
    plot <- generate_scatter_plot_data(params$myData[["CombinedDayRefCoNoPA"]][[x]] , 
                                   ...
                                   paste(x, " - ", params$myData[["CombinedInfoRefCoNoPA"]] %>% 
                                                   dplyr::filter(DeviceID == !!x) %>% dplyr::pull(SiteName)
                                         ,sub , sep = ""),
                                   type = "NO2",
                                   start_date = params$start_date
    )
  ...
  .f = function(x) {
    ...
    plot <- generate_diurnal_boxplot_data(
      ...
      paste(x, " - ", 
                     ...
                     dplyr::filter(DeviceID == !!x) %>% 
                     dplyr::pull(SiteName), sub, sep = ""),
      type = "NO2"
    )
    return(plot)
  }
